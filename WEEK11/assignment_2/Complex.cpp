...
Number *Complex::add(Number * num) {
     ...
     */
     if(num->types() == INTEGER) {
     	Integer *num_int = dynamic_cast<Integer *>(num);
        this->set_val(this->real() + num_int->val(), this->imag());
        delete num_int;
	return this;
    }
    else if (num->types() == FLOAT) {
        Float *num_fl = dynamic_cast<Float *>(num);
        this->set_val(this->real() + num_fl->val(), this->imag()) ;
        delete num_fl;
        return this;
    }
    else if (num->types() == COMPLEX) {
        Complex *num_comp = dynamic_cast<Complex *>(num);
        this->set_val(std::make_pair(this->real() + num_comp->real(),this->imag() + num_comp->imag()));
        delete num_comp;
        return this;
    }
    else {
